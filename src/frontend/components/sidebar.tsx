          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                onClick={() => setMobileOpen(false)}
                className={cn(
                  'flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium text-sm transition-all duration-200 group relative active:scale-98',
                  isActive
              ...
              >
                <Icon className={cn('w-5 h-5 shrink-0 transition-transform group-hover:scale-110 duration-200', isActive ? 'text-white' : 'text-slate-500 dark:text-slate-400')} />
                ...
                {collapsed && (
                  <div className="absolute left-full ml-2 px-2.5 py-1 bg-slate-900 text-white text-xs font-medium rounded-md whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity shadow-lg z-50 animate-scale-in">
                    {item.label}
                  </div>
                )}
              </Link>
